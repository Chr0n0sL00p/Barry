        ...
        for (message in messages) {
            for (record in message.records) {
                ...
                if (tnf == NdefRecord.TNF_WELL_KNOWN && java.util.Arrays.equals(record.type, NdefRecord.RTD_URI)) {
                    ...
                    // If it is our custom app scheme barry://profile/{id}, extract the ID
                    if (uri.startsWith("barry://profile/")) {
                        return uri.substringAfter("barry://profile/").trim()
                    }
                }
